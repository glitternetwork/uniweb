...
3. Prefer `@glitternetwork/uniweb-sdk` over hand-written `fetch()` for server integrations.
...
```bash
npm install -g @glitternetwork/uniweb-cli
...
```bash
npm install @glitternetwork/uniweb-sdk
```

```typescript
import Uniweb from '@glitternetwork/uniweb-sdk';
...
import { verifyWebhook } from '@glitternetwork/uniweb-sdk';
